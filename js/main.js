...
(() => {
  ...
  function getCurrentUserSafe() {
    // Sitede farklı sayfalarda farklı anahtarlar kullanılmış olabilir.
    // Hepsini deniyoruz. (login tarafı: currentUser + isLoggedIn)
    ...
    // Bazı akışlarda sadece isLoggedIn yazılı kalabiliyor.
    // En azından navbar'ı "Hesabım" olarak güncelleyelim.
  ...
  window.addToCart = function(a, b, c) {
    // Her kaynaktan (inline onclick / listener / mobil ghost-click)
    // gelebilecek çift tetiklemeyi engelle.
    try {
      const now = Date.now();
      const sig = typeof a === "object" && a ? (a.id || a.title || a.name || "") : String(a || "");
      if (window.__lastAddToCartAt && (now - window.__lastAddToCartAt) < 350 && window.__lastAddToCartSig === sig) {
        return;
      }
      window.__lastAddToCartAt = now;
      window.__lastAddToCartSig = sig;
    } catch {
      // sessiz geç
    }
  ...
  function initAddToCartFallback() {
    ...
    const handler = (ev) => {
      ...
      if (typeof window.addToCart === "function") {
        // Mobilde bazen click iki kere tetiklenebiliyor (double-tap/ghost-click).
        // Basit bir debounce ile iki eklemeyi engelliyoruz.
        const now = Date.now();
        if (window.__lastAddToCartTs && (now - window.__lastAddToCartTs) < 350) return;
        window.__lastAddToCartTs = now;
    ...
    buttons.forEach((btn) => {
      ...
      // Eğer butonda zaten inline onclick varsa ikinci kez bağlamayalım (ikişer ekleme nedeni).
      if (btn.getAttribute("onclick")) return;

      // Sadece click dinle (touchend + click birlikte iki kez tetikleyebiliyor).
  ...
  function boot() {
    ...
    window.addEventListener("storage", (e) => {
      if (["currentUser", "user", "firebaseUser", "isLoggedIn"].includes(e.key)) updateAuthUI();
    });
  ...
  if (document.readyState === "loading") {
    ...
    // Script bazen async/cached koşullarda DOMContentLoaded sonrası gelebilir.
    boot();
  }
})();
