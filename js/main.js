...
(() => {
  ...
  // ---------- Checkout -> WhatsApp ----------
  // Numara formatı: 905xxxxxxxxx (başında + yok, boşluk yok)
  ...
  function initCheckoutWhatsApp() {
    ...
    // Başka handler'lar submit'i yakalasa bile sayfa yenilenmesin
    form.addEventListener(
      ...
      (e) => {
        ...
        cart.forEach((it) => {
          ...
          lines.push(`- ${it.title} x${it.qty} = ${formatTL(lineTotal)}`);
        });
        lines.push("");
        ...
        const wa = digitsOnly(WHATSAPP_NUMBER);

        if (!wa || wa.length < 10 || wa.includes("X")) {
          alert("WhatsApp numarası ayarlı değil. js/main.js içindeki WHATSAPP_NUMBER değerini kendi numaranla değiştir.");
          return;
        }
